...
const start = catchAsync(
	async (req, res) => {
		const data =
			await attemptService.start(
				getUserId(req),
...
const recordProctorEvent = catchAsync(
	async (req, res) => {
		const data =
			await attemptService.recordProctorEvent(
				getUserId(req),
				getParam(req, "id"),
		...
		return sendResponse(res, {
			statusCode: 201,
			message:
				"Proctor event recorded successfully",
			data,
		});
	},
);

const submit = catchAsync(
	async (req, res) => {
		const data =
